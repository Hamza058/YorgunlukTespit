...
with map_face_mesh.FaceMesh(min_detection_confidence=0.5, min_tracking_confidence=0.5) as face_mesh:
    ...
    while True:
        ...
        if results.multi_face_landmarks:
            ...
            ratio_blink = blinkRatio(frame, mesh_coords, RIGHT_EYE, LEFT_EYE)
            ratio_yawn = yawnRatio(frame, mesh_coords, UPPER_LIPS, LOWER_LIPS)
            # cv.putText(frame, f'ratio {ratio}', (100, 100), FONTS, 1.0, utils.GREEN, 2)
            utils.colorBackgroundText(frame, f'Ratio : {round(ratio_blink, 2)}', FONTS, 0.7, (30, 100), 2, utils.PINK,
                                      utils.YELLOW)

            if Total_yawn==1 or TOTAL_BLINKS==1:
                tic=str(time.perf_counter()).split('.')
                toc=int(tic[0])-4

                if toc > 60:
                    if Total_yawn>5 or TOTAL_BLINKS>30:
                        utils.colorBackgroundText(frame, f'1dk da Esneme sayınız: {Total_yawn}', FONTS, 0.7, (30, 130), 2)
                        utils.colorBackgroundText(frame, f'1dk da Göz kırpma sayınız: {TOTAL_BLINKS}', FONTS, 0.7, (30, 160), 2)
